<template>
  <view class="page">
    ...
    <view
      class="stats-row"
      v-if="activeBorrows.length > 0 || stats.history > 0"
    >
    ...
    <!-- 区块标题 -->
    <view class="section-header" v-if="activeBorrows.length > 0">
      ...
      >
        ...
        <view class="card-dates">
          ...
          <view class="date-item">
            ...
            <text class="date-value">{{ item.dueDateStr }}</text>
          </view>
          <view class="date-item">
        ...
        <view class="card-action">
          <view class="renew-btn" @click="handleRenew(item)">
            <text>续借</text>
          </view>
        </view>
      </view>
...
const loadData = () => {
  ...
  const active = raw.filter((b) => {
    const status = b.status || "borrowing";
  ...
  activeBorrows.value = active.map((b) => {
    ...
    return {
      ...
      dueDateStr: formatDate(b.dueDate),
      daysLeft,
      overdue: daysLeft < 0,
    };
  });
...
const handleRenew = (item) => {
  if (item.renewCount >= 2) {
    uni.showToast({ title: "已达到最大续借次数", icon: "none" });
    return;
  }
